...
we use as the reference implementation (see \S\ref{sec:mapping} for
the full capability classification) --- the commitment structure is as follows. A
...
We measure the gap $\Delta t = t_{\text{observed}} - t_{\text{commit}}$ on an
Astria shared-sequencer devnet ($N=15$ blocks). The empirical mean is
$2{,}199$\,ms ($\pm44$\,ms std), far exceeding typical MEV searcher
latency of 10--50\,ms~\cite{daian2020flash}. This is the structural window
in which a rational provider can observe $\tau_1$'s execution and front-run
$\tau_2$ (Figure~\ref{fig:timing-cdf}).

\paragraph{Counterfactual replay (\S\ref{sec:meas-replay}).}
We replay 12 months of Optimism and Base block-fee data ($N=1{,}300$ blocks
combined) to estimate the tail index of the option-value distribution.
The Hill estimator gives $\hat{\alpha} = 1.18$ (OP+Base combined, 95\% CI
$[0.98, 1.47]$), confirming $\hat{\alpha} > 0$ and the validity of
Theorem~\ref{thm:bond-insufficient}'s $\rho_{\min} > 0$ bound. The empirical
violation rate on Base at $B_{\max} = 0.05$\,ETH is 2.0\%, matching the
Theorem~2 lower bound of 2.6\%.
