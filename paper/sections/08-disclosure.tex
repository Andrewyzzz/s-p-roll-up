...
\caption{Disclosure summary.
  All disclosures were made at least 90 days prior to submission.
  Specific dates, OTS proof hashes, and git commit anchors are
  withheld for blind review and will be provided in the camera-ready version
  via the public artifact repository~\cite{artifact-repo}.}
\label{tab:vendor-responses}
\begin{tabular}{@{}llll@{}}
\toprule
Vendor & Category & Disclosed & Outcome \\
\midrule
Astria       & $D_1$ & $\geq 90$ days prior & pending \\
Espresso     & $D_1$ & $\geq 90$ days prior & pending \\
Radius       & $D_1$ & $\geq 90$ days prior & precautionary \\
Primev       & $D_2$ & $\geq 90$ days prior & pending \\
Chainbound   & $D_2$ & $\geq 90$ days prior & archived (Feb 2026) \\
\bottomrule
\end{tabular}
\end{table}

\noindent Full vendor correspondence, OTS proof hashes, and git commit
anchors are provided in the camera-ready version (Appendix~\ref{app:disclosure}).

\subsection{Timestamping and Verifiability}
\label{sec:disclosure-timestamps}

All disclosure attempts are timestamped using a two-layer scheme:
(1)~per-attempt OpenTimestamps proofs anchored to the Bitcoin blockchain
via~\cite{opentimestamps}, and (2)~immediate git commits to the artifact
repository~\cite{artifact-repo}. The combination provides tamper-evident
evidence that disclosure content was fixed prior to submission.

Specific proof hashes, commit identifiers, and file paths are withheld
in this submission to preserve author anonymity. The complete verifiable
record will be released upon acceptance as part of the artifact package.
