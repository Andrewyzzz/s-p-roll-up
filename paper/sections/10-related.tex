...
preconfirmations~\cite{PLACEHOLDER-ethresear-1, PLACEHOLDER-ethresear-2,
PLACEHOLDER-ethresear-3}. These works focus primarily on L1 single-chain
preconfirmations (a validator committing to include a transaction in the
next block). Our contribution extends the analysis to the cross-rollup setting,
where the commitment spans two independent STFs, and provides the first
formal proof that A2 closure requires capability (i) or (ii) regardless of
bond size.
% TODO (Task 1.5): replace PLACEHOLDERs with actual citations from annotated bibliography.
...
inclusion~\cite{PLACEHOLDER-flashbots-cross-rollup}. These proposals identify
...
The smart contract security literature~\cite{PLACEHOLDER-sc-analysis} has
