...
\paragraph{2. Silva and Livshits~\cite{arxiv2410.11552}.}
Silva and Livshits study the economics of atomic execution in shared
sequencers, showing that switching to atomic execution does not always
improve arbitrage profits. Their focus is on whether atomicity is
\emph{worth providing} (economic optimality for MEV extraction).
Our contribution is orthogonal: we study whether atomic execution
\emph{can be provided at all} without capability~(i) or~(ii)
(structural impossibility), and calibrate the economic cost of the
gap that results. Silva and Livshits do not address the A2/A4
separation, the capability taxonomy, or the empirical violation-rate
calibration that constitute our main contributions.

\paragraph{3. CCS 2025 Denial of Sequencing~\cite{ccs2025dos}.}
This work identifies denial-of-sequencing as an attack vector on shared
sequencer architectures: an adversary withholds inclusion of a committed
bundle. Our contribution is orthogonal: we study the case where the
sequencer faithfully includes the bundle but the inclusion commitment still
admits free-option attacks due to the execution gap. Both attacks exploit
properties of lazy sequencers; ours focuses on selective exercise of the
option value embedded in a commitment, not refusal to honor inclusion.
