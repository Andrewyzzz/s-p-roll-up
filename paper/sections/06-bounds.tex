...
\noindent\textbf{Empirical estimates (full dataset, $N = 300$ per chain).}
\begin{align*}
  \hat{\alpha}_{\text{OP}} &= 1.35 \quad \text{(Hill } k{=}100\text{, 95\% CI: [1.09, 1.69])} \\
  \hat{\alpha}_{\text{Base}} &= 1.19 \quad \text{(Hill } k{=}100\text{, 95\% CI: [1.00, 1.48])} \\
  \hat{\alpha}_{\text{Arb}} &= 0.47 \quad \text{(Hill } k{=}100\text{; infinite-mean regime)} \\
  \hat{\alpha}_{\text{OP+Base}} &= 1.18 \quad \text{(combined, 95\% CI: [0.98, 1.47])}
\end{align*}
All chains confirm $\hat{\alpha} > 0$, satisfying Theorem~\ref{thm:bond-insufficient}.
For the primary calibration we use the OP+Base combined estimate
$\hat{\alpha} = 1.18$.
For $B_{\max} = 0.05$\,ETH (mev-commit reference):
$\Pr[V > 0.05] \sim C \cdot (0.05)^{-1.18} > 0$ for all finite $B_{\max}$,
confirming $\rho_{\min} > 0$.
